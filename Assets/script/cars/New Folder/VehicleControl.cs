...
public class VehicleControl : MonoBehaviour
{
    ...
    void Awake()
    {
        ...
        void SetupWheelPhysics(WheelComponent w, WheelSetting setting)
        {
            ...
            col.radius = setting.Radius;
            col.mass = setting.Weight;
    ...
    : slip = Mathf.Lerp(slip, 0.01f, 0.02f);
            ...
            foreach (WheelComponent wheel in wheels)
            {
                    ...
                    else
                    {
                        ...
                        if (!brake && accel != 0 && NeutralGear == false)
                        {
                                ...
                                (speed < carSetting.LimitBackwardSpeed && currentGear == 0))
                            {
                                ...
                                collider.motorTorque = curTorqueCol * 0.9f + newTorque * 1.0f;
                            }
                            else
                            {
                                collider.motorTorque = 0;
                ...
                if (Maincar)
                {
                    if (brake || slip2 > 2.0f)
                    {
                        collider.steerAngle = Mathf.Lerp(collider.steerAngle, steer * wheel.maxSteer, 0.02f);
                    }
                    else
                    {

                        float SteerAngle = Mathf.Clamp(speed / carSetting.maxSteerAngle, 1.0f, carSetting.maxSteerAngle);
                        collider.steerAngle = steer * (wheel.maxSteer / SteerAngle);
